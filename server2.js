...
const FLAGS = { easy: "CTF{filter_bypass_achieved}", medium: "CTF{xss_executed_successfully}", hard: "CTF{rce_simulated_correctly}", god: "CTF{rooted_through_mind_twist}", expert: "CTF{chained_bypass_mastermind}", crazy: "CTF{insane_chain_triggered}" };
...
`;app.get('/', (req, res) => { res.send(htmlWrapper("Radzz CTF Web", <p class="mb-4">Choose your difficulty:</p> <div class="flex flex-col gap-2"> <a href="/easy" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-xl text-center">Easy</a> <a href="/medium" class="bg-yellow-600 hover:bg-yellow-700 px-4 py-2 rounded-xl text-center">Medium</a> <a href="/hard" class="bg-red-600 hover:bg-red-700 px-4 py-2 rounded-xl text-center">Hard</a> <a href="/god" class="bg-purple-800 hover:bg-purple-900 px-4 py-2 rounded-xl text-center">GOD MODE</a> <a href="/expert" class="bg-pink-700 hover:bg-pink-800 px-4 py-2 rounded-xl text-center">EXPERT</a> <a href="/crazy" class="bg-black hover:bg-gray-900 px-4 py-2 rounded-xl text-center">CRAZY MODE</a> </div>)); });

// [Semua route Easy, Medium, Hard, God, Expert tidak dimuat ulang di sini agar tidak duplikatif.]
...
// CRAZY MODE — paling sulit, chaining + bypass + nonce + logic trap app.get('/crazy', (req, res) => { res.send(htmlWrapper("CRAZY MODE", <p class="mb-4">Welcome to the abyss. Only legends unlock this.</p> <p>Step 1: Solve the encoded puzzle:</p> <code class="block bg-gray-700 p-2 rounded mt-2">ZnVuY3Rpb24gY3JhY3soKXtjb25zb2xlLmxvZygnRmxhZzogQ1RGe2Jhc2U2NF9nZWVrc30nKX0=</code> <p class="mt-2">Send result to: <code>/crazy/step1</code> via POST with key <code>answer</code></p>)); });

app.post('/crazy/step1', (req, res) => { const { answer } = req.body; if (answer && answer.trim() === "Flag: CTF{base64_geeks}") { res.send(htmlWrapper("CRAZY STEP 2", <p>Good. Now crack the XSS+SQLi+Logic gate at:</p> <a href="/crazy/step2" class="underline text-red-400">/crazy/step2</a> <p class="text-xs mt-2">Hint: Your payload must execute AND return SQLi-style input to trigger the flag.</p>)); } else { res.send(htmlWrapper("Wrong", <p>That's not it.</p>)); } });

app.get('/crazy/step2', (req, res) => { const q = req.query.q || ''; const sanitized = q.replace(/(script|onerror|alert|iframe|img|src|data:|fetch|select|union|from|where|or|and|--|;|'|")/gi, '[x]'); res.send(htmlWrapper("CRAZY STEP 2", <p>Payload received. If it's right, the system will know.</p> <form> <input name="q" placeholder="Inject payload..." class="bg-gray-700 p-2 rounded mb-2 w-full" /> <button type="submit" class="bg-red-800 hover:bg-red-900 px-4 py-2 rounded">Submit</button> </form> <div class="mt-4" id="sink">${sanitized}</div> <script nonce="cr4zyNonce"> const val = new URLSearchParams(window.location.search).get("q"); if (val && val.includes("admin") && val.includes("1=1") && /<\\w+.*on[a-z]+=/.test(val)) { document.getElementById("sink").innerHTML = val; setTimeout(() => { document.write('<h1>CRAZY FLAG: ' + "${FLAGS.crazy}" + '</h1>'); }, 500); } </script> <p class="text-xs mt-2">CSP nonce simulation active. Payload must be JS-executable AND satisfy logic.</p>)); });

app.listen(PORT, '0.0.0.0', () => { console.log(CTF backend running: http://0.0.0.0:${PORT}); });
